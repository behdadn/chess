...
// Portability.h)
int strcmp_ignore(const char *s,
                  const char *t); // return 0 if case insensitive match
...
class ChessPosition : public ChessPositionRaw {
  public:
    ...
    bool operator<(const ChessPosition &other) const {
        ...
        if (!temp)
            return ((int)groomed_enpassant_target() <
...
class ChessRules : public ChessPosition {
  public:
    ...
    void GenLegalMoveList(MOVELIST *list, bool check[MAXMOVES],
                          bool mate[MAXMOVES], bool stalemate[MAXMOVES]);
  ...
  protected:
    ...
    void ShortMoves(MOVELIST *l, Square square, const lte *ptr,
                    SPECIAL special);
